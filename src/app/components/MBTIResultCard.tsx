  ...
  return (
    <section className="flex flex-col items-center w-full mt-8">
      ...
      >
        ...
        {/* 한글 이름(크게) + 가타카나(괄호) */}
        <div style={{ marginTop: 8 }} className={`text-2xl font-extrabold mb-1 transition-opacity duration-500 ${isVisible ? "opacity-100" : "opacity-0"}`}>{result.korean}</div>
      ...
      {/* MBTI 설명 */}
      <div className="w-full max-w-xs bg-gray-50 rounded-lg p-4 mb-4 transition-shadow duration-300 hover:shadow-md">
        ...
        <div className="text-sm text-gray-600 whitespace-pre-line text-left pr-[5px]" style={{ textAlign: 'left' }}>{result.persona?.description}</div>
      </div>
      {/* 복사/공유 버튼 */}
